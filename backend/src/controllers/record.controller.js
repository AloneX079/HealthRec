...
const getBasicInfo = asynchandler(async (req, res) => {
  ...
  const basicInfo = await Record.findOne({
    ...
    "fullName dateOfBirth gender bloodGroup phoneNumber address maritalStatus ethnicityRace smokingAlcohol"
  );
  if (!basicInfo) {
...
const getMedicalHistory = asynchandler(async (req, res) => {
  ...
  const medicalHistory = await Record.findOne({
    ...
    "medicalHistory familyMedicalHistory allergies immunizationHistory surgeriesUndergone"
  );
  if (!medicalHistory) {
    throw new apierror(
      404,
      "No Medical History Found! ERR:record.controller.l26"
    );
  }
  return res
    .status(200)
...
const getInsuranceInfo = asynchandler(async (req, res) => {
  ...
  if (!insuranceInfo) {
    throw new apierror(
      404,
      "No Insurance Info Found! ERR:record.controller.l50"
    );
  }
  return res
    .status(200)
    .json(
      new apiresponse(
        200,
        insuranceInfo,
        "Insurance Info Fetched Successfully!"
      )
    );
});
...
const getEmergencyContact = asynchandler(async (req, res) => {
  ...
  if (!emergencyContact) {
    throw new apierror(
      404,
      "No Emergency Contact Found! ERR:record.controller.l62"
    );
  }
  const {
...
const getVisitHistory = asynchandler(async (req, res) => {
  ...
  if (!prescobjid) {
    throw new apierror(
      404,
      "No Visit History Found! ERR:record.controller.l75"
    );
  }
  const visitHistory = [];
  ...
  return res
    .status(200)
    .json(
      new apiresponse(200, visitHistory, "Visit History Fetched Successfully!")
    );
});
...
const upBasicInfo = asynchandler(async (req, res) => {
  ...
  const updateData = {
    ...
    emergencyContactPhone:
      {
        ...(req.body.primaryrespondername && {
          primaryrespondername: req.body.primaryrespondername,
        }),
        ...(req.body.primaryresponderphone && {
          primaryresponderphone: req.body.primaryresponderphone,
        }),
        ...(req.body.secondaryrespondername && {
          secondaryrespondername: req.body.secondaryrespondername,
        }),
        ...(req.body.secondaryresponderphone && {
          secondaryresponderphone: req.body.secondaryresponderphone,
        }),
      } || undefined,
  };
  ...
  if (
    filteredUpdateData.primaryrespondername &&
    !fullNameRegex.test(filteredUpdateData.primaryrespondername)
  )
    throw new apierror(
      400,
      "Invalid Primary Responder Name! ERR:record.controller.l116"
    );
  if (
    filteredUpdateData.primaryresponderphone &&
    !phoneRegex.test(filteredUpdateData.primaryresponderphone)
  )
    throw new apierror(
      400,
      "Invalid Primary Responder Phone Number! ERR:record.controller.l118"
    );
  if (
    filteredUpdateData.secondaryrespondername &&
    !fullNameRegex.test(filteredUpdateData.secondaryrespondername)
  )
    throw new apierror(
      400,
      "Invalid Secondary Responder Name! ERR:record.controller.l120"
    );
  if (
    filteredUpdateData.secondaryresponderphone &&
    !phoneRegex.test(filteredUpdateData.secondaryresponderphone)
  )
    throw new apierror(
      400,
      "Invalid Secondary Responder Phone Number! ERR:record.controller.l122"
    );
  if (
    filteredUpdateData.fullName &&
    !fullNameRegex.test(filteredUpdateData.fullName)
  )
    throw new apierror(400, "Invalid Full Name! ERR:record.controller.l116");
  if (
    filteredUpdateData.dateOfBirth &&
    !dateOfBirthRegex.test(filteredUpdateData.dateOfBirth)
  )
    throw new apierror(
      400,
      "Invalid Date of Birth! ERR:record.controller.l118"
    );
  ...
  if (
    filteredUpdateData.bloodGroup &&
    !bloodGroupRegex.test(filteredUpdateData.bloodGroup)
  )
    throw new apierror(400, "Invalid Blood Group! ERR:record.controller.l122");
  if (
    filteredUpdateData.phoneNumber &&
    !phoneNumberRegex.test(filteredUpdateData.phoneNumber)
  )
    throw new apierror(400, "Invalid Phone Number! ERR:record.controller.l124");
  if (
    filteredUpdateData.address &&
    !addressRegex.test(filteredUpdateData.address)
  )
    throw new apierror(400, "Invalid Address! ERR:record.controller.l126");
  if (
    filteredUpdateData.maritalStatus &&
    !maritalStatusRegex.test(filteredUpdateData.maritalStatus)
  )
    throw new apierror(
      400,
      "Invalid Marital Status! ERR:record.controller.l128"
    );
  if (
    filteredUpdateData.ethnicityRace &&
    !raceRegex.test(filteredUpdateData.ethnicityRace)
  )
    throw new apierror(
      400,
      "Invalid Ethnicity/Race! ERR:record.controller.l130"
    );
  if (
    filteredUpdateData.smokingAlcohol &&
    !smokingAlcoholRegex.test(filteredUpdateData.smokingAlcohol)
  )
    throw new apierror(
      400,
      "Invalid Smoking/Alcohol! ERR:record.controller.l132"
    );
  if (
    filteredUpdateData.insuranceProvider &&
    !fullNameRegex.test(filteredUpdateData.insuranceProvider)
  )
    throw new apierror(
      400,
      "Invalid Insurance Provider! ERR:record.controller.l134"
    );
  if (
    filteredUpdateData.heightInCm &&
    !heInCmRegex.test(filteredUpdateData.heightInCm)
  )
    throw new apierror(400, "Invalid Height! ERR:record.controller.l136");
  if (
    filteredUpdateData.weightInKg &&
    !weInKgRegex.test(filteredUpdateData.weightInKg)
  )
  ...
  if (!checkExists) {
    ...
    if (!record) {
      throw new apierror(
        500,
        "Error Creating Record! ERR:record.controller.l138"
      );
    }
    return res
      .status(201)
    ...
    if (!record) {
      throw new apierror(
        500,
        "Error Updating Record! ERR:record.controller.l151"
      );
    }
    return res
      .status(200)
...
const upMedicalHistory = asynchandler(async (req, res) => {
  ...
  if (!checkExists) {
    ...
    if (!record) {
      throw new apierror(
        500,
        "Error Creating Record! ERR:record.controller.l199"
      );
    }
    return res
      .status(201)
...
const upPatientVitals = asynchandler(async (req, res) => {
  ...
  const hrRegex = /^\d{2,3}$/;
  if (
    filteredUpdateData.LastBloodPressureInMmHg &&
    !bpRegex.test(filteredUpdateData.LastBloodPressureInMmHg)
  )
    throw new apierror(
      400,
      "Invalid Blood Pressure! ERR:record.controller.l441"
    );
  if (
    filteredUpdateData.LastHeartRateInBpm &&
    !hrRegex.test(filteredUpdateData.LastHeartRateInBpm)
  )
  ...
  if (!checkExists) {
    ...
    if (!record) {
      throw new apierror(
        500,
        "Error Creating Record! ERR:record.controller.l447"
      );
    }
    return res
      .status(201)
...
const upVisitHistory = asynchandler(async (req, res) => {
  ...
  if (!checkExists) {
    ...
    if (!pres)
      throw new apierror(
        500,
        "Error Creating Prescription! ERR:record.controller.l536"
      );
    const record = await Record.create({
      pid: patid,
    ...
    if (!record)
      throw new apierror(
        500,
        "Error Creating Record! ERR:record.controller.l542"
      );
    return res
      .status(201)
      .json(new apiresponse(201, record, "Record Created Successfully!"));
    ...
    if (!pres)
      throw new apierror(
        500,
        "Error Creating Prescription! ERR:record.controller.l552"
      );
...
const getPresPhar = asynchandler(async (req, res) => {
  ...
  if (prescs.length < 1) throw new apierror(404, "No Prescriptions Exists");
  return res
    .status(200)
